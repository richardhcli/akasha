...
|---|---|---|---|
| T13.1 | Accept `task_state` on `PATCH /v1/nodes/{id}` | TODO | Closes the HTTP-surface gap `docs/acceptance.md` row 8 and pre-mvp T10.2c both disclosed. Binding narrowest reading: `docs/spec-questions.md` **T13.1**. Watch the omitted-vs-`null` distinction — `store.commit_node`'s `task_state` is sentinel-guarded (`_UNSET_TASK_STATE`), so an omitted field must not be forwarded as an explicit `None`. Regenerate `docs/api-snapshot/openapi.json` in the same change (§6.3 gate). |
| T13.2 | `reconcile.project_node_change()` helper | TODO | Library-only, no call site (T13.3 wires it) — deliberately split so the logic lands separately from the risky wiring. Reuses the existing `ProjectionIndex.owner` and `Reconciler.on_change`; must not invent a second projection path, and must not project unfiled nodes (they stay unfiled and keep counting toward `GET /sync/export`'s `unfiled_node_count`). File-disjoint from T13.1 — the two are the natural first parallel cohort. |
...
| T13.4 | CLI `akasha set --task-state open\|done` | TODO | Pure HTTP client of T13.1's field; `--json`/`--dry-run`/exit codes come free from `_mutate`. `set` is already a discovered mutating verb so the dry-run meta-test set is unchanged, but add a `set_task_state` `DryRunCase` variant and list its id in the meta-test's variant-exclusion set next to `rm_with_redirect`. Shares `cli/main.py` with all of M14's CLI tasks — sequential. Depends on T13.1. |
...
| Task | Goal | Status | Notes |
|---|---|---|---|
| T14.1 | CLI `akasha neighborhood ID` / `akasha history ID` | TODO | Both endpoints shipped in M4 and are reachable today from no surface but raw HTTP. Read-only → routed through `_request`, **no** dry-run table entry (the meta-test discovers mutating verbs only). ASCII-only default output (pre-mvp T9.9 was a real Windows `UnicodeEncodeError` crash from exactly this). Binding narrowest reading for all of M14's CLI work: `docs/spec-questions.md` **T14.2**. |
...
| T14.5 | Web UI: make the 1-hop neighborhood navigable | TODO | Today `renderNeighborhood` prints `src -edge_type-> dst` as plain text with no links and no bodies — debug-plan D8 fixed exactly this class of defect for search/review/sync and never reached the neighborhood. Group by direction then edge type (`composes` first), fetch each neighbor once for type+body preview (bounded by the 1-hop set — **no** endpoint change to carry bodies), show `facet_binding` including `*`. Extend `tests/integration/test_ui_node_links.py`; do not add a second links test file. Needs Chromium. |
| T14.6 | Web UI: facets-from-spans link form on the node view | TODO | Builds the never-built UI half of M7's own DoD ("facets-from-spans capture flow in API/**UI**") — grep-verified 2026-08-05, no UI code has ever POSTed `/v1/edges`, so the only way a facet is born from a span today is a hand-written HTTP call. This is PRD R8's designed fix for facet bootstrap and `facet_coverage` is a **gating** dogfood metric. Binding narrowest reading: `docs/spec-questions.md` **T14.6**. Test must assert a real facet appears on the target and `facet_coverage` moves. Needs Chromium. Depends on T14.5. |
