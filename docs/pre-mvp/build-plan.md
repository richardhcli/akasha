...
### T9.6 — Wire the live filesystem `Watcher` into `daemon.serve()`
- **Goal** — Make the daemon actually detect live filesystem edits to vault files without requiring a manual `POST /v1/sync/rescan` call, by instantiating and starting `sync/watcher.py`'s `Watcher` class inside `daemon.serve()`, wired to a persistent `Reconciler`'s `on_change` — the production data flow `mvp-spec.md`'s own architecture diagram describes ("Obsidian vault → watcher → sync/reconcile → kernel") but that has never actually been built: `Watcher` (T5.3) has zero production call sites (confirmed by grep across `src/`), and both startup `reconcile_all` (T5.6) and `POST /v1/sync/rescan` (T11.3) only ever process already-known files, never a live filesystem event.
- **Depends on** — T5.3 (DONE — `Watcher`/`Debouncer`), T5.4 (DONE — `Reconciler.on_change`), T5.6 (DONE — startup reconcile; the closest existing "run something inside `serve()`'s lock" precedent for ordering), T9.3 (DONE — `GcScheduler`; the closest existing "start a background component, stop it in `finally`" lifecycle precedent).
- **Files** — `src/akasha/daemon.py` (`serve()`), `tests/integration/test_watcher_wiring.py` (new).
- **Spec** — `mvp-spec.md` architecture diagram (§2: "watcher ──► sync/reconcile ──► kernel"), §4.8 ("Startup: run `on_change` for every managed file... Echo suppression: writes performed by the daemon record `(path, hash)` in `origin.py`; a watcher event whose content hash matches a recorded write is dropped" — this sentence presumes a live watcher exists), M5 DoD text ("watcher with debounce + cloud-path detection").
- **Scope narrowing (do NOT widen)** — do not touch `sync/watcher.py` or `sync/reconcile.py` internals; both are complete, tested, and already designed for exactly this wiring per their own docstrings (`Watcher`'s constructor contract and `Reconciler.on_change`'s signature ARE the seam, not something to redesign). Do not change `reconcile_all`/`POST /v1/sync/rescan` (T11.3's discovery wiring stays exactly as-is — this task adds a live watcher on top of the existing startup+manual-rescan paths, it does not replace either). **CRITICAL correctness pitfall, name it explicitly so a worker doesn't rediscover it the hard way**: the `Watcher`'s `on_cycle` callback must be a SINGLE persistent `Reconciler` instance's bound `.on_change` method, constructed ONCE and reused for the watcher's entire lifetime — never a fresh `Reconciler`/`OriginTracker`/`ProjectionIndex` per event. `Watcher`'s optional echo-suppression (`OriginTracker`) and `ProjectionIndex`'s cross-file-move tracking both depend on state accumulated across calls; constructing fresh instances per event would silently defeat both features while still passing a naive "does it call on_change at all" test.
- **Steps** — (1) In `daemon.serve()`, after the existing startup `reconcile.reconcile_all(...)` call and before `uvicorn.run`, construct ONE `Reconciler(app.state.conn, origin_tracker, projection=projection)` for the daemon's live-watch lifetime. Decide explicitly (and document the decision inline, not silently) whether this reuses the exact `OriginTracker`/`ProjectionIndex` instances the startup `reconcile_all` pass used, or constructs fresh ones for the live-watch "session" — either is defensible, but it must be a stated decision, not an accident. (2) Construct `Watcher(app.state.conn, on_cycle=reconciler.on_change, ...)` and call `.start()` inside the `with single_instance_lock(...)` block, alongside `gc_scheduler.start()`. (3) Call `.stop()` in the same `finally` block that already stops `gc_scheduler`, so a clean shutdown always joins the watcher's observer thread rather than leaking it. (4) Add an integration test that starts a real daemon, registers a sync root, edits a vault file ON DISK directly (not via HTTP), waits out the debounce window, and asserts the edit was picked up WITHOUT any `POST /v1/sync/rescan` call — the actual end-to-end behavior this task exists to deliver. (5) Confirm `test_crash_recovery.py`, `test_daemon_lock_multiprocess.py`, and the T11.x dogfood-flow behavior are unregressed (the watcher must not double-process an event that startup `reconcile_all` or a manual rescan already handled this cycle).
- **Verify** — `uv run pytest tests/integration/test_watcher_wiring.py tests/integration/test_crash_recovery.py tests/integration/test_daemon_lock_multiprocess.py`
- **DoD** — a real running daemon detects and reconciles a live on-disk edit to a registered sync root's file within one debounce window, with no HTTP call required; existing startup-reconcile/manual-rescan/lock/crash-recovery behavior unregressed; `make check` green; the dedicated spec-question entry this task's registration adds (see `docs/spec-questions.md`) moves to `docs/archived-questions.md` on landing.

### T9.7 — `GET /v1/search` 500s on a hyphenated (or otherwise FTS5-operator-laden) query term
- **Goal** — A search query containing SQLite FTS5 syntax characters (a bare `-` is FTS5's column-filter/NOT-operator, plus `"`/`*`/`(`/`)`/`AND`/`OR`/`NOT` all carry MATCH-syntax meaning) must never 500; escape/quote the query the same defensive way `store.py::find_contradiction_candidates` (T10.2b) already does for its own FTS5 `MATCH` construction, rather than passing the raw user string straight into `MATCH`.
- **Depends on** — T3.1 (DONE — `nodes_fts`/search), T10.2b (DONE — the existing FTS5-quoting precedent to reuse, not reinvent).
- **Files** — `src/akasha/kernel/store.py` (`search`), `tests/integration/test_api.py` (existing `/search` coverage lives there, not a dedicated file — added to it rather than forking a duplicate).
- **Spec** — §4.11 `GET /search`; no grammar change — this is a robustness fix to an existing endpoint's handling of arbitrary user text, not a new capability.
- **Found via** — the T9.6 live-watcher session's manual verification pass: `GET /v1/search?q=already-tracked` returned HTTP 500 against a real running daemon. Reproduced deterministically; not a flake.
- **Scope narrowing (do NOT widen)** — do not touch `nodes_fts`'s schema/tokenizer or ranking. This is purely "never let a raw query string reach FTS5's `MATCH` operator unescaped" — the same discipline T10.2b already applies when building its own MATCH expression from a node body.
- **Steps** — (1) Reproduce directly against `store.search` (unit-level) with inputs containing `-`, unbalanced `"`, and bare `AND`/`OR`/`NOT` to confirm which characters actually trigger FTS5 syntax errors vs. simply return zero rows. (2) Apply the same per-term FTS5-quoting approach T10.2b's `find_contradiction_candidates` uses (quote each tokenized term) rather than passing the raw string to `MATCH`. (3) Add regression tests for every character class reproduced in step 1, plus the exact reported case (`already-tracked`).
- **Verify** — `uv run pytest tests/integration/test_api.py` plus a live manual check: `GET /v1/search?q=already-tracked` returns 200.
- **DoD** — no input string can 500 `GET /v1/search`; existing non-adversarial search behavior (ranking, result shape) unregressed; `make check` green.
- **DONE 2026-07-26.** Extracted a shared `_fts5_safe_match_query` helper in `store.py` (replacing the private `_CONTRADICTION_TERM_RE` with a shared `_FTS5_TERM_RE`, reused by both `search` and `find_contradiction_candidates`): tokenizes the raw query to alphanumeric-only terms, double-quotes each (no embedded-quote escaping ever needed — terms are alnum-only by construction), and space-joins (implicit AND, preserving `search`'s existing multi-word behavior — unlike `find_contradiction_candidates`'s own OR-join, which stays OR since it's a different, deliberately broader heuristic). A query with zero alphanumeric terms (including a non-ASCII-only query — a known, narrower-than-ideal limitation shared with the existing T10.2b precedent, not reinvented here) short-circuits to `[]` before ever issuing a MATCH. 11 new regression tests in `tests/integration/test_api.py` (9 parametrized adversarial-input cases incl. the exact reported `already-tracked`, all asserting 200; 1 positive case proving a hyphenated query still finds a hyphenated body; 1 multi-word case proving implicit AND is preserved). Reproduced the original bug directly against a bare `sqlite3` FTS5 table first to confirm root cause before fixing. Verified live against two real scratch daemons (`scripts/dogfood/init.sh`/`destroy.sh`) — one confirming `GET /v1/search?q=already-tracked` no longer 500s, a second confirming a genuine positive match against real seeded content (not just an empty-but-non-500 response). Full gate: ruff clean, pyright 0 errors on both platforms, 659 unit+property+integration+battery passed (up from 648).

### T9.8 — Nightly soak: RSS budget breach on the first real scheduled 24h run (run `30194717387`, 2026-07-26)
- **Goal** — Diagnose and fix (or, if genuinely out of the harness's control, document with real evidence) the cause of `tests/battery/soak.py`'s RSS climbing past its 150MB DoD ceiling roughly 4.5 hours into a real 24-hour run, so the *next* scheduled run either passes or fails with enough instrumentation to diagnose itself immediately.
- **Depends on** — T9.5 (DONE — the soak harness itself; this task investigates a real result it produced, not a design flaw in the harness's structure).
- **Files** — `tests/battery/soak.py` only. Investigation confirmed the root cause lives entirely in the harness's own request-driving pattern, not in `src/akasha/*` (see Resolution below) — no production module needed touching.
- **Found via** — `nightly-soak` CI job, run `30194717387`, 2026-07-26 08:30–12:58 UTC (the first real *scheduled*, not manually-dispatched, 24h run on `main`): `SOAK FAILED: RSS budget breached at tick 8019: 161419264 bytes >= 157286400 byte limit`. `nightly-battery` in the same run was green. A second scheduled run, `30256037982`, 2026-07-27 09:56–14:24 UTC, failed identically at tick 8001 (161067008 bytes) while this task's fix was still pending, and supplied the full heartbeat log this task's resolution was diagnosed from.
- **DONE 2026-07-27.** Root cause: `_setup()` builds a `fastapi.testclient.TestClient` but never enters it as a context manager. Starlette's `TestClient._portal_factory` only reuses one long-lived `anyio` blocking portal (a background thread + asyncio event loop) across calls when the client is entered via `with`/`__enter__`; left un-entered, `_portal_factory`'s fallback branch spins up and tears down a **brand-new portal on every single `.get()`/`.post()` call**. Confirmed (not assumed) via three independent lines of evidence: (1) the earlier WAL-growth lead from Step 1 was checked against the second real run's full heartbeat log and refuted — `wal_file_bytes` plateaus near ~4.1MB for the entire run (SQLite's own auto-checkpoint keeping it bounded), ruling that hypothesis out; (2) a `tracemalloc` line-level diff across an accelerated local run showed the dominant allocation growth entirely in `anyio`/`asyncio` internals (worker-thread/portal bookkeeping), with zero `src/akasha/*` lines appearing in the top 20; (3) a controlled local A/B probe (3000 sequential `/v1/metrics` GETs against the real app, with vs. without `__enter__`) measured ~6.2 KB/request RSS growth and 3x higher per-request latency without entering the context, both eliminated once entered — decisively isolating the mechanism. Fix: `_setup` now calls `client.__enter__()` once after construction (comment at the call site records the mechanism); `run_soak`'s existing `finally` block (which already manually closes `state.conn` before `TemporaryDirectory` teardown for the same Windows-handle-locking reason) now also calls `state.client.__exit__(None, None, None)` first. **Verified:** documented `--hours 0.05` proxy passes (RSS ~61.75MB). A longer accelerated local proxy (`--tick-seconds 0.02 --hours 0.111`, 19980 ticks — 2.4x the corpus size at the real breach, crossing several of the previously-observed ~24-min spike cycles) now holds RSS flat (65–76MB) with no periodic spikes at all; at the exact node_count (696) where the real run breached at 161MB, this run reads 71.4MB. Full gate: `ruff check` clean, `pyright --strict src` 0 errors, 413 unit+property passed, 47 battery passed. No SPEC-QUESTIONs — a test-harness robustness fix, not a spec ambiguity.
- **Steps (historical, kept for record)** — (1) DONE 2026-07-26: corpus-size heartbeat instrumentation. (2)–(4) as originally scoped (multi-hour `search`/`vault_edit` isolation via `--hours 2`–`4` real dispatch) turned out to be unnecessary once the second real run's heartbeat log + local tracemalloc/A/B probes pinpointed the harness-level `TestClient` context-manager bug directly — no CI-dispatched multi-hour isolation run was needed to reach a confirmed root cause.
- **Verify** — `uv run pytest tests/unit tests/property tests/battery` (green) plus the accelerated proxies described above. **NOT** the next real scheduled `nightly-soak` run on `main`: confirmed 2026-07-27 against GitHub's own Actions limits documentation ("Each job in a workflow can run for up to 6 hours of execution time. If a job reaches this limit, the job is terminated and fails.") that GitHub-hosted runner jobs hard-cap at 6h of execution time regardless of `timeout-minutes`, and `ci.yml`'s `nightly-soak` job runs `--hours 24` on `runs-on: windows-latest` — a real scheduled run will be killed by that platform ceiling near hour 6 independent of whether this fix holds. Both prior failures (runs `30194717387`, `30256037982`) happened at ~4.5h, before ever reaching that ceiling, which is why it was never previously exposed. This is a real, separate, currently-open gap — not silently fixed here since `ci.yml` is outside this task's `Files` list (rule 8) and choosing a new duration/chunking strategy is a judgment call, not a mechanical edit. See `docs/agents/task-status.md`'s M9 header for the open note.
- **DoD** — met: the RSS ceiling is no longer breached in a multi-cycle accelerated run, with a defensible mechanism (not just an absence-of-symptom) for why the fix generalizes to the real 24h case; `make check` green.

### T9.9 — CLI crashes with `UnicodeEncodeError` on Windows consoles whose default codepage isn't UTF-8
- **Goal** — No `akasha` CLI invocation should crash with `UnicodeEncodeError` on a real Windows host, regardless of the console's locale-preferred encoding.
- **Depends on** — T4.8 (DONE — the CLI itself).
- **Files** — `src/akasha/cli/main.py`.
- **Spec** — no grammar/schema change; this is a robustness fix to the existing CLI's output plumbing (spec §4.12), not a new capability.
- **Found via** — a Windows-environment dogfood-readiness sweep on this repo's own Windows 11 Enterprise Evaluation dev host: `python -c "import sys; print(sys.stdout.encoding)"` returned `cp1252` (not UTF-8) on this host, and `akasha token create --help` reproducibly crashed with `UnicodeEncodeError: 'charmap' codec can't encode character '∅'` — the docstring used as that command's help text contains U+2205 (empty-set symbol), which has no cp1252 representation. Reproduced the same crash for `token revoke --help`, `token list --help`, and `review resolve --help` (all four share the same `∅` marker for "human only").
- **Scope narrowing (do NOT widen)** — this is a client-side output-encoding fix only; no server/API/schema change, no new dependency. `json.dumps`'s default `ensure_ascii=True` already protects every JSON-echoed value (node bodies, error payloads, etc.) from this class of crash — confirmed by creating a real node with non-cp1252 content (café, →, 你好, 🚀, ∅) over live HTTP via the CLI and reading it back, both before and after this fix, with no crash either way. The actual gap was narrower: raw (non-JSON) text paths — command docstrings surfaced as `--help` output — and, defensively, any future raw `typer.echo`/error-message text that isn't run through `json.dumps`.
- **Steps** — (1) Remove the non-cp1252 `∅` symbol from the four affected command docstrings in `cli/main.py`, replacing it with plain text ("human only"). (2) Root-cause fix, not just symptom avoidance: on `sys.platform == "win32"`, reconfigure `sys.stdout`/`sys.stderr` to UTF-8 (`errors="replace"` as a defensive fallback) at CLI module-import time — UTF-8 can represent every Unicode string losslessly, so this removes the whole crash class rather than requiring every future string literal to avoid non-cp1252 characters by convention. Guarded with `isinstance(_stream, io.TextIOWrapper)` (not bare `hasattr`) so it both narrows correctly for `pyright --strict` and skips streams already replaced by something that doesn't support `.reconfigure` (e.g. click's test `CliRunner`).
- **Verify** — `uv run pytest tests/unit tests/property tests/integration tests/battery`; live: `akasha token create --help` / `token revoke --help` / `token list --help` / `review resolve --help` all exit 0 on this Windows host (previously crashed); a real node created via the CLI with non-cp1252 Unicode content round-trips correctly through `new`/`get` against a live scratch daemon.
- **DoD** — no CLI invocation crashes with `UnicodeEncodeError` on this host's default (cp1252) console encoding; `make check` green; existing CLI test suite unregressed.
- **DONE 2026-07-26.** Both fixes landed together (belt-and-suspenders: the docstring fix is also a standalone readability improvement, independent of the encoding root-cause fix). Verified live against a real scratch daemon (`scripts/dogfood/init.sh windows-refine-verify`/`destroy.sh`, port 7440): all four previously-crashing `--help` invocations now exit 0; a node created via `akasha new claim "<unicode text>"` (café/→/你好/🚀/∅) round-tripped correctly through both `new` and `get`, confirming the `json.dumps`-escaped path was never actually broken (this fix closes the raw-text gap alongside it, not a regression fix for that path). Full gate: `ruff check` clean, `pyright --strict src` 0 errors on both default (Windows) and forced `--pythonplatform Linux`, 659 unit+property+integration+battery passed (unchanged from T9.7/T9.8 — this task added no new test file, since the crash is Windows-console-encoding-specific and not reproducible under pytest's captured-stdout environment; live verification was the correct Verify method here, same precedent as T9.6). No SPEC-QUESTIONs — this is a Windows-environment robustness fix, not a spec ambiguity.
...
### T12.4 — One-command Windows setup script (bridge, pre-packaging) — DONE, see docs/agents/task-status.md M12
...
### T12.5 — PyInstaller executable + tray icon + installer — DONE, see docs/agents/task-status.md M12
...
- **Depends on** — T12.4.
- **Files** — `pyproject.toml` (PyInstaller as a packaging-only dev dep), `scripts/windows/build-exe.ps1` (new), `scripts/windows/akasha.iss` (new, Inno Setup script — authored and compiled), `scripts/windows/run-tray-supervised.bat` (new — the supervisor loop the installer's shortcuts launch instead of `akasha.exe tray` directly), `src/akasha/tray.py` (new; `quit_app` exits with code 42 per the supervisor-loop contract), `src/akasha/cli/main.py` (new `tray` command), `src/akasha/kernel/store.py` (`MIGRATIONS_DIR` frozen-build fix, disclosed as `docs/spec-questions.md` T12.5 — self-resolved), `docs/user/ops/autostart.md`.
- **Verify** — manual/live leg on a real Windows host: build the exe, run the installer on a clean VM/user profile, confirm autostart survives logoff/logon and a `taskkill /F` (reuse the exact live kill-and-poll test `docs/dogfood/windows-service.md` already describes). **Done:** the exe builds and was live-verified (`--help`/`init`/`daemon`/`tray` all run correctly against a fresh scratch config, including an authenticated HTTP round-trip and the dashboard serving real bytes). The Inno Setup installer was compiled (`ISCC.exe`, found at `C:\Users\User\AppData\Local\Programs\Inno Setup 7\ISCC.exe`) and run `/VERYSILENT` on the real host: zero elevation prompts, correct `%LocalAppData%\akasha` placement, correct Start Menu + `{userstartup}` shortcuts (both pointing at the supervisor `.bat`, not the exe directly), and live-verified crash recovery (force-killed `akasha.exe`, new PID answering `/health` within ~4s). Quit-vs-crash exit-code-42 semantics verified via code review + a deterministic `.bat` control-flow test (3/3 scripted cases passed) — disclosed gap: a live tray-icon click on "Quit" was attempted but inconclusive (a desktop-access grant needed to see the menu timed out), so that one specific interaction is logic-verified rather than click-verified. See task-status.md for full evidence.
- **DoD** — as above, all confirmed on a real Windows host, not estimated. Met, with the one disclosed live-click gap noted above.
