...
- **Files** — `tests/battery/soak.py` (heartbeat instrumentation; possibly the fix itself, depending on root cause), the actual production module the root cause lives in (TBD by investigation — do not guess it in advance).
- **Found via** — `nightly-soak` CI job, run `30194717387`, 2026-07-26 08:30–12:58 UTC (the first real *scheduled*, not manually-dispatched, 24h run on `main`): `SOAK FAILED: RSS budget breached at tick 8019: 161419264 bytes >= 157286400 byte limit`. `nightly-battery` in the same run was green.
- **Evidence gathered so far (not yet a diagnosis)** — heartbeat RSS was flat (~45–51MB) between spikes across the whole run, but three periodic spikes were observed roughly every ~720 ticks (~24 min real time): ~134.5MB (tick 6570), ~144.3MB (tick 7320), ~161.4MB/breach (tick 8019) — i.e. a periodic transient that grows in *peak* size roughly with elapsed corpus size, not a monotonically-growing baseline. This pattern (flat floor, escalating periodic peak) is NOT the signature of a classic unbounded-retention leak; it is consistent with a periodic operation whose cost scales with accumulated corpus (leading hypothesis: SQLite FTS5's periodic segment automerge on `nodes_fts`, given `search`/`vault_edit` actions are among the soak's heaviest-weighted and the only other known FTS5-related defect this same session (T9.7) also lives in that subsystem — **unconfirmed, needs the isolation step below before treating as fact**).
- **Steps** — (1) Add corpus-size fields (node count, commit count, `nodes_fts` row count, DB file size on disk) to `soak.py`'s existing heartbeat log line, so any future run (pass or fail) carries enough data to diagnose itself without needing a second 24h run. (2) Run the accelerated proxy (`--hours` scaled up enough to cross several of the ~24-minute spike cycles, e.g. `--hours 2`–`4`, not the DoD's own `0.05`) locally with the `search` action weight forced to 0 (no FTS5 reads) and separately with `vault_edit` forced to 0 (no FTS5 writes via rescan's re-indexing), to isolate which action class drives the escalating peak. (3) Once isolated, either fix the root cause directly (if it is a real, controllable inefficiency) or, if it is inherent to sustained sustained-write FTS5 corpora at a rate far exceeding realistic human dogfood usage (do the math: 24/7 human editing at the soak's synthetic tick rate implies num-edits/day no real single-human dogfooder would produce), document that finding with the actual numbers in `docs/acceptance.md` and adjust the DoD's RSS ceiling or measurement window accordingly — but only after step 2's isolation data supports that conclusion, not as a first resort. (4) Re-run a real accelerated proxy long enough to cross at least 4 of the periodic cycles and confirm the fix (or documented non-fix) holds.
- **Verify** — `uv run python tests/battery/soak.py --hours 2` (or whatever duration step 2 settles on) passes with the new heartbeat fields visible in the log; the next real *scheduled* `nightly-soak` run on `main` is the ultimate confirming evidence and should be checked once it lands.
- **DoD** — either the RSS ceiling is no longer breached in a multi-cycle accelerated run with a defensible explanation of why the fix generalizes to the real 24h case, or (if not fixable at this scope) the finding is honestly documented in `docs/acceptance.md`/`docs/agents/task-status.md` with real numbers rather than the DoD being silently weakened; `make check` green.
- **Step 1 DONE 2026-07-26** (instrumentation only — user explicitly chose this over kicking off a multi-hour diagnostic run this session). `_corpus_stats(state)` in `soak.py` samples `node_count`/`commit_count`/`fts_row_count`/`db_file_bytes`/`wal_file_bytes` at heartbeat cadence (not every tick, to avoid the instrumentation itself adding soak overhead) and on the RSS-breach path. Verified via `--hours 0.05`. **Real lead found by that verification, still unconfirmed:** `wal_file_bytes` grew continuously across the whole 3-minute proxy with no checkpoint observed (`db_file_bytes`, the main file, stayed flat at 4096 bytes throughout) — suggests WAL checkpointing may not be firing under this harness's sustained load, a candidate explanation for a periodic-cost signature if checkpoints are instead large/infrequent/expensive rather than the previously-assumed FTS5-automerge hypothesis. Steps 2–4 (the multi-hour isolation experiment) remain open.
- **Not eligible for `fleet-orchestrator`/autonomous dispatch — marked `BLOCKED` in `task-status.md`, not `TODO`.** Steps 2–4 as written need multiple hours of real wall-clock soak execution per run (e.g. `--hours 2`–`4`, run twice to isolate `search` vs `vault_edit`). A `fleet-worker` subagent's Bash tool is capped well below that per call, and `fleet-worker.md`'s own "Before you return" section requires killing any background process before returning rather than leaving it running across turns — a single worker invocation cannot complete this task's literal `Verify` command. **Correct dispatch mechanism, if picked up:** trigger the existing `nightly-soak` CI job's `workflow_dispatch` input (`soak_hours`) via `gh workflow run`, then check results in a later session once it completes — not a synchronous local run inside one worker turn. (Adding `search`/`vault_edit`-toggle CLI flags to `soak.py` to actually isolate the cause, and a matching `workflow_dispatch` input, is real follow-up work but was deliberately NOT built speculatively this session — build it when someone is actually ready to run the isolation experiment, not before.)
