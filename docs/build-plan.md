...
- **Files** — `src/akasha/kernel/store.py` (`search`), `src/akasha/api/routes/search.py` (if the query is normalized there instead — pick exactly one seam, do not split escaping across both), `tests/integration/test_search.py` (or a new file if none exists yet).
...
- **Steps** — (1) Reproduce directly against `store.search` (unit-level) with inputs containing `-`, unbalanced `"`, and bare `AND`/`OR`/`NOT` to confirm which characters actually trigger FTS5 syntax errors vs. simply return zero rows. (2) Apply the same per-term FTS5-quoting approach T10.2b's `find_contradiction_candidates` uses (quote each tokenized term, OR-join) rather than passing the raw string to `MATCH`. (3) Add regression tests for every character class reproduced in step 1, plus the exact reported case (`already-tracked`).
- **Verify** — `uv run pytest tests/integration/test_search.py` (or wherever the new/existing search tests live) plus a live manual check: `GET /v1/search?q=already-tracked` returns 200.
