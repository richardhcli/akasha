...
- **Resolution:** resolved 2026-08-02 — the user directed this UX audit directly in this session (same authorization pattern as D4/D5). Implementation is deliberately deferred to a CLI-capable agent that can run the full gate on a real Windows host per build-plan rule 0.7; see `docs/onboarding-ux-report.md` for the proposed M12 tasks (T12.1–T12.6) and T12.1's own remaining sub-decision (bootstrap-token transport: CLI verb vs. endpoint).
